...
def move(rawfname,aNumber,captureDir,mmrepo,archRepoDir):
	...
	if os.path.isdir(processingDir):
		...
		if sh[-32:] == dh[-32:]:
			output = subprocess.Popen(['python',os.path.join(mmrepo,'hashmove.py'),os.path.join(captureDir,rawfname + ".wav"),endDir],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
			foo,err = output.communicate()
