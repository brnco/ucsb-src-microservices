	...
	with open(txtinfo) as arb:
		...
		for x in process: #result is list
			...
			if '1/4-inch Half Track Mono' in x:
				...
				#print ffstr
				ffstr = "ffmpeg -i " + rawfname + ".wav " + zerostr + onestr
				return ffstr,revstr01
			if '1/4-inch Full Track Mono' in x:
				ffstr = "ffmpeg -i " + rawfname + ".wav " + silencestr + " -ac 1 -acodec pcm_s24le " + rawfname + "-downmix.wav"
				return ffstr,revstr01
			else:
				#reverse_fAB
				if "rev_fAB" in x:
				...
				if 'dblspd_fAB' in x:
					dblstr01 = ',"asetrate=192000"'
				ffstr = "ffmpeg -i " + rawfname + ".wav " + silencestr + hlvstr01 + dblstr01 + " -acodec pcm_s24le " + rawfname + "-processed.wav"
				return ffstr, revstr01
	return
