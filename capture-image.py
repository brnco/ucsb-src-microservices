...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="captures image from first-connected camera"
    )
    parser.add_argument(
        '-nj', action='store_true',
        default=False, dest='nj',
        help='run with National Jukebox file destinations, on //svmwindows/special/78rpm'
    )
    ...
    if args.nj:
        target = os.path.join(
            conf.NationalJukebox.VisualArchRawDir, file_path.name
        )
    #elif args.phi:
