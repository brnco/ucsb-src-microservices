...
def main():
	...
	parser.add_argument('-m', '--mode', dest='m', choices=['batch', 'single'], default=False, help='single mode, for processing a single transfer')
	parser.add_argument('-i', '--input', dest='i', help="the rawcapture file.wav to process, single mode only")
	args = parser.parse_args()
	###END INIT###
	if args.m == 'batch':
		for dirs, subdirs, _ in os.walk(captureDir):
