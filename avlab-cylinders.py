		...
		for s in subdirs:
			...
			with cd(os.path.join(captureDir, s)):
				...
				if not os.path.exists(startObj): #if we don't have a b.wav file in the dir we should just stop
					...
					subprocess.call(['python',os.path.join(mmrepo,'makebroadcast.py'),startObj,'-ff','-n','-c']) #calls makebroadcast.py and tells it to insert 2s fades and normalize to -1.5db
